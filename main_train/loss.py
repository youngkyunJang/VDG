...
class HardNegativeNCE(torch.nn.Module):
    """
    Hard-Negative NCE loss for contrastive learning.
    ...
    def forward(
        self,
        composed_embs: torch.Tensor,
        target_embs: torch.Tensor,
        temp,
    ):

        batch_size = composed_embs.size(0)
        # computation of the similarity matrix
        sim_matrix = composed_embs @ target_embs.T  # (batch_size, batch_size)
